...
def token_required(roles=None):
    ...
    def decorator(func_to_guard):
        ...
        def decorated(*args, **kwargs):
            ...
            # DEBUG LOGGING
            print(f"[token_required] Checking for cookie: {cookie_name}")
            print(f"[token_required] All cookies received: {request.cookies}")
            print(f"[token_required] Token found: {token[:20] if token else 'None'}...")
            print(f"[token_required] Request origin: {request.headers.get('Origin')}")
            print(f"[token_required] Request path: {request.path}")
