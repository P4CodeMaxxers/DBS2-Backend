...
**Your backend is running on:** `localhost:8403` ✅
...
**Change the port from `8585` to `8403`:**
...
// AFTER (Correct):
const javaURI = "http://localhost:8403";
```
...
export const javaURI = "http://localhost:8403";  // ← Make sure it's 8403!
...
Edit `assets/js/api/config.js` and change `8585` → `8403`
...
```bash
curl http://localhost:8403/api/post/all
# Should return: []
```
...
| Service | Port | Status |
|---------|------|--------|
| **Flask Backend** | **8403** | ✅ Correct - Use this! |
...
1. **Backend is on 8403** - This is confirmed and working ✅
2. **Don't change backend** - It's correct!
3. **Change frontend config** - Update to port 8403
...
Once you update the port to `8403`:
...
```javascript
// Backend API Configuration
export const javaURI = "http://localhost:8403";
...
# Find and replace 8585 with 8403
find assets -name "*.js" -type f -exec sed -i '' 's/8585/8403/g' {} +
...
| What | Where | Change |
|------|-------|--------|
| **Backend** | Flask (running) | Port **8403** ✅ |
| **Frontend Config** | `config.js` | Change to **8403** ❌→✅ |
...
Just update that one line in your frontend config from `8585` to `8403` and everything will work!
